...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "SHOPDECREMENT":
      ...
      const item2 = {
        ...state.elItem,
        transQty: state.elItem.transQty < 2 ? 1 : state.elItem.transQty - 1,
        total: state.elItem.transQty * state.elItem.price,
      };
    ...
    case "DECREMENT":
      const tempCart2 = state.transArray
        .map((item) => {
    ...
    case "CARTDECREMENT":
      const tempCart7 = state.cartArray
        .map((item) => {
          if (item.id === action.payload) {
            console.log(item);
            return {
    ...
    case "CARTFIELDCHANGE":
      const item3 = {
        ...state.elItem,
        transQty: Number(action.payload),
        total: state.elItem.price * action.payload,
      };
    ...
    case "REMOVECARTITEM":
      const currentCart = state.cartArray.filter(
    ...
    case "remove":
      const newArray = state.transArray.filter(
    ...
    case "GETCARTTOTAL":
      const { cartAmount, totalCart } = state.cartArray.reduce(
        (cartTotal, cartItem) => {
