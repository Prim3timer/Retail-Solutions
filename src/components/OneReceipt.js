...
const OneReceipt = ({ setOneReceipt }) => {
  ...
  const { currentUsers, setIsRotated, falseRotated, currency, bizName } =
    ...
    try {
      ...
      if (response) {
        const newRes = response.data.map((item) => {
          ...
          const latestReceipt = response.data.filter(
            (receipt) => receipt.cashierID === auth.picker,
          );
          const reverseReceipt = latestReceipt.reverse();
          console.log(auth.picker);
          const oneTrans = response.data.find(
            (item) => item._id === localStorage.getItem("memTransaction"),
          );
  ...
  return !currentTrans ? (
    ...
    <div className="one-receipt" onClick={() => setIsRotated(false)}>
      {currentTrans && (
        <article
          style={{
            display: "flex",
        ...
        >
          ...
          {currentTrans.goods.map((good) => {
            return (
              ...
              >
                ...
                {good.storage && (
                  <p>
                    storage: {good.storage} {good.storage == 1000 ? "TB" : "GB"}
                  </p>
                )}
                <p>
                  Sub Total: {currency}
          ...
          <p>
            card ending in: ...{currentTrans.last4 ? currentTrans.last4 : ""}
          </p>
          <h4
            style={{
              textAlign: "left",
