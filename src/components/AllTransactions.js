...
const AllTransactions = () => {
  ...
  const { falseIsRotated, currency, oneShow, bizName } =
  ...
  const getItems = async () => {
    try {
      ...
      if (response) {
        ...
        const filterate = response.data.filter((inner) =>
  ...
  const handleRemove = async () => {
    ...
    const newGraw =
      response && allTransi.filter((item) => item._id !== state.id);
    console.log("removed");
  ...
  return !state.getNames.length ? (
    <h2 className="all-transactions-header">Loading...</h2>
  ) : (
    <div className="all-transactions-body" onClick={falseIsRotated}>
      ...
      </form>
      {
        <div onClick={remainDelete}>
          ...
          >
            All Reciepts ({allTransi.length})
          </h3>
          {allTransi.map((item) => {
            return (
              <section>
                <Link to={"/one-receipt"} style={{ textDecoration: "none" }}>
                  <article
                    id="receipts"
                    style={{
                      display: "flex",
