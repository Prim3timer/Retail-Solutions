  ...
  const getItem = async () => {
    ...
    try {
      ...
      if (goods) {
        ...
        const picsOnly = newGoods && newGoods.img.filter((item) => item.name !== 'no image')
          ...
          <article className="inner-single-item">
            ...
            <section className="single-image-container">
              ...
              <article className="shop-big-img">
                <p onClick={lower}><FaChevronLeft /></p>
                <p onClick={higher}> <FaChevronRight /></p>
              </article>
              <div className="single-item-array">
                {justPics && justPics.map((image, i) => {
                  console.log(image)
                  return (

                    <img className={i === index ? 'onview' : 'no-onview'} src={`${picUrl}/images/${state.elItem.name}/${image.name}`} onClick={() => handleIndex(i)} alt={image.name} />
                  )
                })}
              </div>
            ...
            <div className="single-item-texts">
              <p className="description">{readMore ? state.elItem.description : `${state.elItem.description.substring(0, 37)}`}<span className="show-more" onClick={() => setReadMore(!readMore)}>{readMore ? ' show less' : state.elItem.description.length >= 36 ? '...read more' : ''}</span></p>
              <section
                className="qty-cont"
              >
