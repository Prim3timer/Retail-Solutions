...
const Inventory = ({ mark, setMark }) => {
  ...
  const getTrans = async () => {
    try {
      ...
      const filterate = items.filter((inner) =>
        inner.name.toLowerCase().includes(state.search.toLowerCase()),
      );
      dispatch({ type: "items", payload: filterate });

      if (search2) {
        const stockFilter =
          filterate && filterate.filter((item) => item.qty <= search2);
  ...
  return !state.items ? (
    <h2 className="inventory-spec">...Loading</h2>
  ) : (
    <section
    ...
    >
      ...
      >
        <h2 id="verify-header">Unauthorized!</h2>
      ...
      >
        ...
        <div className="edit-action">
          <button
            ...
            onClick={(e) => dispatch({ type: "isEdit", payload: false })}
          >
            Cancel
          </button>
          <button
