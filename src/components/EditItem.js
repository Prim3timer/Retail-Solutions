...
const EditItem = () => {
  ...
  const handleDeletePic = async (pic, id, index) => {
    try {
      ...
      const response = await axiosPrivate.delete(
        `/delete-pic/${getPic.name}?name=${item.name}&id=${item._id}`,
      );
      if (response) {
  ...
  const getItem = () => {
    ...
    if (currentItem) {
      ...
      currentItem.availablePrices.length > 1 &&
        setSecondPrice(currentItem.availablePrices[1]);
      currentItem.availablePrices.length > 2 &&
        setThirdPrice(currentItem.availablePrices[2]);
      currentItem.availablePrices.length > 3 &&
        setFourthPrice(currentItem.availablePrices[3]);
    ...
    if (file && file.size > 2000000) {
      ...
      try {
        ...
        const response = await axios.patch(
          `/items/pic/${item.name}?fiveArray=${JSON.stringify(fiveArray)}&initialPic=${initialPic}&id=${item._id}`,
          formData,
        );
        if (response) {
  ...
  const imageFunc = async () => {
    try {
      if (file && file.size > 2000000) {
        ...
        const currentBackItem = backendItems.data.items.find(
          (it) => it._id === item._id,
        );
        if (currentBackItem) {
  ...
  const handleItemDelete = async () => {
    ...
    try {
      const response = await axiosPrivate.delete(
        `/items/delete/${item._id}?name=${item.name}`,
      );
      if (response) {
        navigate("/item-list");
      }
  ...
  return (
    <div className="edit-an-item" onClick={remainDelete}>
              ...
              <div className="edit-item-image" key={pic.id}>
                ...
                {pic.name === "no image" ? (
                  ...
                  <div className="the-icons">
                    <label onClick={() => handleDeletePic(pic, pic.id, index)}>
                      {" "}
                      <FontAwesomeIcon
                ...
                {pic.name === "no image" ? (
                  <div className="input-icon">
                    {id === pic.id && file ? (
                      <p
                        className={
                          file.size <= 2000000 ? "pic-name" : "pic-no-name"
                        }
                      >
                        {file.size <= 2000000 ? file.name : ""}
                      </p>
                    ) : (
                      ""
                    )}
                    <br />{" "}
                    {id !== pic.id || !file || file.size > 2000000 ? (
                      <label className="plus" htmlFor="addImage">
                    ...
                    {state.errMsg ===
                      "Please limit file sizes to 2MB. Thank you." || !file ? (
                      <input
                        type="file"
                  ...
                  <img
                    ...
                    src={`${picUrl}/images/${item.name}/${pic.name}`}
                    alt={pic.name}
                  />
                )}
                {/* if file has a value and id = id of the element  in picArray and  both success and isLoading is false */}
                {file && file.size <= 2000000 && !isLoading ? (
                  <button
                    className={
                      id === pic.id && file.size <= 2000000
                        ? "show-button"
                        : "hide-button"
                    }
                    onClick={() => handleUpload(pic.id)}
                    //
                  >
                    upload image
                  </button>
                ...
                {isLoading && id === pic.id ? (
                  <p className="uploading-image">uploading...</p>
                ) : (
                  ""
                )}
              </div>
      ...
      <form onSubmit={(e) => e.preventDefault()} className="item-update-form">
        <label htmlFor="name">
          name
          <input
            type="text"
        ...
        {state.success || msgBackup ? (
          <h2 className="delete">{state.errMsg}</h2>
        ) : (
          ""
        )}
      </form>
      ...
      <div className={state.cancel ? "delete" : "no-delete"}>
        ...
        >
          {" "}
          Delete {item.name && item.name} from items
        </h3>
        <article
          style={{
            display: "flex",
