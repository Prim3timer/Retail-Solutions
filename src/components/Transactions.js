...
const Transactions = () => {
  ...
  const now = new Date();
  const { auth, setAtHome, falseIsRotated, currency, items, picUrl } =
    ...
    try {
      if (inputRef.current.value) {
        ...
        const currentItem = items.find(
          (name) => `${name.name}` === inputRef.current.value,
        );
  ...
  return (
    <div className="trans-cont" onClick={falseIsRotated}>
      ...
      <div id="trans-item-cont">
        {!state.transArray.length ? (
          ...
          state.transArray.map((item, index) => {
            ...
            return (
              <section key={index} id="trans-item">
                <section className="trans-name-and-img">
                  ...
                  />
                  <h5 className="trans-item-name">{item.name}</h5>
                </section>
                ...
                <article id="flex-article">
                  ...
                  />
                  <span> {item.unitMeasure.split(" ")[1].slice(1, -1)}</span>
      ...
      <section id="trans-verify-section">
        <div className={state.cancel ? "display-veryfier" : "hide-veryfier"}>
          <h3
            id="verify-header"
            style={{
              margin: ".5rem",
          ...
          <article className="inside-veryfier">
            <button onClick={remain}>No</button>
            <button
              onClick={clearer}
              style={{
                backgroundColor: "red",
